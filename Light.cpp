...
void Light::generateLightRay(LocalGeo& local, Ray* ray, Color* c) {
	...
	float t_max = 1000.f;
	if (type == 0) {
		d = new Vertex(-dir->x, -dir->y, -dir->z);
	} else {		
		...
		d = new Vertex(d_temp.x, d_temp.y, d_temp.z);
	}
	float ratio = 0.0f;
	Vertex* p = new Vertex(local.pos->x + ratio*d->x, local.pos->y + ratio*d->y, local.pos->z + ratio*d->z);
	Ray* r = new Ray(p, d, 0.1f, t_max);
	*ray = *r;
	*c = *color;
}